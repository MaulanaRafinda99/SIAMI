...
class model_asesor extends CI_Model
{
    ...
    // public function getinstansi($where)
    // {
    //     $query =
    //         "
    //  SELECT * FROM instansi
    //  WHERE id_instansi=$where
    //      ";
    ...
    public function getinstansi()
    {
        ...
        $this->db->select('*');
        $this->db->from('user_access_data');
        $this->db->where('akun', $id_user);
        $access_data_user = $this->db->get()->row_array();

        if ($access_data_user) {
            $id_instansi = $access_data_user['id_instansi'];
    ...
    // public function getsndikti2($where, $siklus)
    // {
    //     $query = "
    //     SELECT * FROM data_ami
    //     WHERE id_instansi='$where' AND id_siklus='$siklus'
    // ";
    //     return $this->db->query($query)->result_array();
    // }
    ...
    public function getsndikti2($where, $siklus)
    {
        ...
        if (isset($access_data_user['id_instansi'])) {
            $id_instansi = $access_data_user['id_instansi'];
        } else {
            echo "No access data found for user.";
            return [];
        }

        $this->db->select('*');
        $this->db->from('data_ami');
        $this->db->where('id_instansi', $id_instansi);
        $this->db->where('id_siklus', $siklus);
